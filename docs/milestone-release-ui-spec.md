...
Handoff spec for implementing milestone-gated partial fund release in this frontend.
Describes the screens, states, and exact contract calls needed, on top of the existing
funded/refundable all-or-nothing flow.
...
Contract: `apps/contracts/crowdfund/src/lib.rs` in the
[`stellar-raise-contracts`](https://github.com/Crowdfunding-DApp/stellar-raise-contracts)
repo (see branch `feature/milestone-release`). All calls below are Soroban contract
invocations against the campaign's `CrowdfundContract` instance, signed via Freighter.
